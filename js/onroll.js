Trackball = function() {
     ...
     * @private
     */
    this.rollStart_ = new THREE.Vector3(0, 0, 1);
     ...
     * @private
     */
    this.center_ = new THREE.Vector2(0,0);
 ...
 * @private
 */
Trackball.prototype.initInFrame_ = function(frameSize) {
 ...
 * @private
 */
Trackball.prototype.convertClientPoint_ = function(clientPoint) {

    return ( new THREE.Vector2(clientPoint.x - this.center_.x, clientPoint.y - this.center_.y) );
};
 ...
 * @private
 */
Trackball.prototype.projectOnTrackball_ = function(point) {
    ...
    var trackballPoint = new THREE.Vector3(point.x, point.y, zValue);
 ...
 */
Trackball.prototype.startAtPointInFrame =
 ...
 *     sin(angle/2).
 */
Trackball.prototype.rollToPoint = function(dragPoint) {
    var rollTo =new THREE.Vector2();
    rollTo = this.convertClientPoint_(dragPoint);
    if ((Math.abs(this.rollStart_.x - rollTo.x) <
        Trackball.DOUBLE_EPSILON) &&
    ...
    var invStartMag = 1.0 / this.rollStart_.length();//
    var invEndMag = 1.0 / rollEnd.length();

    // cos(a) = (s . e) / (||s|| ||e||)
    var cosAng = this.rollStart_.dot(rollEnd) * invStartMag * invEndMag;
    // sin(a) = ||(s X e)|| / (||s|| ||e||)
    var sinAng = rollVector.length() * invStartMag * invEndMag;
 ...
 */
Trackball.prototype.handleStartDrag =
 ...
 *     this handler.
 */
Trackball.prototype.handleEndDrag =
